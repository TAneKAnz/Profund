#include <stdio.h>
#include <windows.h>
#include <time.h>
#define scount 20
#define screen_x 80
#define screen_y 25
HANDLE rHnd;
HANDLE wHnd;
DWORD fdwMode;
CHAR_INFO consoleBuffer[screen_x * screen_y];
COORD bufferSize = { screen_x,screen_y };
COORD characterPos = { 0,0 };
SMALL_RECT windowSize = { 0,0,screen_x - 1,screen_y - 1 };
COORD star[scount];
int rColor = 7, posx = 40, posy = 12, hit = 10;
bool play = true;

int setConsole(int x, int y)
{
	wHnd = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleWindowInfo(wHnd, TRUE, &windowSize);
	SetConsoleScreenBufferSize(wHnd, bufferSize);
	return 0;
}
void fill_buffer_to_console()
{
	WriteConsoleOutputA(wHnd, consoleBuffer, bufferSize, characterPos, &windowSize);
}
void clear_buffer()
{
	for (int y = 0; y < screen_y; ++y)
	{
		for (int x = 0; x < screen_x; ++x)
		{
			consoleBuffer[x + screen_x * y].Char.AsciiChar = ' ';
			consoleBuffer[x + screen_x * y].Attributes = 7;
		}
	}
}
void init_star()
{
	for (int i = 0; i < scount; i++)
	{
		star[i] = { short(rand() % screen_x), short(rand() % screen_y) };
	}
}
void check_hit(int i)
{
	if (star[i].Y == posy && star[i].X >= posx - 2 && star[i].X <= posx + 2)
	{
		star[i] = { short(rand() % screen_x),1 };
		hit--;
		if (hit == 0)
		{
			play = false;
		}
	}
}
void star_fall()
{
	for (int i = 0; i < scount; i++) 
	{
		check_hit(i);
		if (star[i].Y >= screen_y - 1) 
		{
			star[i] = { short(rand() % screen_x),1 };
		}
		else 
		{
			star[i] = { star[i].X, short(star[i].Y + 1) };
		}
	}
}
void fill_ship_to_buffer()
{
	consoleBuffer[posx - 2 + screen_x * posy].Char.AsciiChar = '<';
	consoleBuffer[posx - 1 + screen_x * posy].Char.AsciiChar = '-';
	consoleBuffer[posx + screen_x * posy].Char.AsciiChar = hit+47;
	consoleBuffer[posx + 1 + screen_x * posy].Char.AsciiChar = '-';
	consoleBuffer[posx + 2 + screen_x * posy].Char.AsciiChar = '>';
	for (int i = -2; i < 3; i++)
	{
		consoleBuffer[posx + i + screen_x * posy].Attributes = rColor;
	}
}
void fill_star_to_buffer()
{
	for (int i = 0; i < scount; i++)
	{
		consoleBuffer[star[i].X + screen_x * star[i].Y].Char.AsciiChar = '*';
		consoleBuffer[star[i].X + screen_x * star[i].Y].Attributes = rColor;
	}
}
int setMode()
{
	rHnd = GetStdHandle(STD_INPUT_HANDLE);
	fdwMode = ENABLE_EXTENDED_FLAGS | ENABLE_WINDOW_INPUT |
		ENABLE_MOUSE_INPUT;
	SetConsoleMode(rHnd, fdwMode);
	return 0;
}

int main()
{
	srand(time(NULL));
	setConsole(screen_x, screen_y);
	init_star();
	DWORD numEvents = 0;
	DWORD numEventsRead = 0;
	setMode();
	while (play)
	{
		star_fall();
		clear_buffer();
		fill_star_to_buffer();
		fill_ship_to_buffer();
		fill_buffer_to_console();
		GetNumberOfConsoleInputEvents(rHnd, &numEvents);
		if (numEvents != 0) {
			INPUT_RECORD* eventBuffer = new INPUT_RECORD[numEvents];
			ReadConsoleInput(rHnd, eventBuffer, numEvents, &numEventsRead);
			for (DWORD i = 0; i < numEventsRead; ++i) {
				if (eventBuffer[i].EventType == KEY_EVENT && eventBuffer[i].Event.KeyEvent.bKeyDown == true)
				{
					if (eventBuffer[i].Event.KeyEvent.wVirtualKeyCode == VK_ESCAPE)
					{
						play = false;
					}
					else if (eventBuffer[i].Event.KeyEvent.uChar.AsciiChar == 'c')
					{
						rColor = rand() % 15 + 1;
					}
				}
				else if (eventBuffer[i].EventType == MOUSE_EVENT)
				{
					posx = eventBuffer[i].Event.MouseEvent.dwMousePosition.X;
					posy = eventBuffer[i].Event.MouseEvent.dwMousePosition.Y;
					if (eventBuffer[i].Event.MouseEvent.dwButtonState & FROM_LEFT_1ST_BUTTON_PRESSED)
					{
						rColor = rand() % 16;
					}
				}
			}
			delete[] eventBuffer;
		}
		Sleep(100);
	}
	return 0;
}